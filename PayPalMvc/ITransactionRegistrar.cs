...
namespace PayPalMvc {
	public interface ITransactionRegistrar {
        ...
        /// </summary>
        /// <param name="uniqueCode">can be ID or Code used for cancel url and reference for getting order info</param>
        /// <param name="allowNote">Allow buyer to add note to seller default 1. 0 to not allow</param>
        /// <param name="noShipping">Value use to indicate if shipping is required</param>
        /// <param name="currencyCode">Currency Code to use for sale</param>
        /// <param name="amount">Total amount of sale</param>
        /// <param name="itemTotalAmount">Item total amount of sale</param>
        /// <param name="taxAmount">Tax amount of sale</param>
        ...
        SetExpressCheckoutResponse SendSetExpressCheckout(string uniqueCode, string allowNote, string noShipping, string currencyCode, decimal amount, decimal itemTotalAmount, decimal taxAmount, string description, string trackingReference, string serverURL, List<ExpressCheckoutItem> purchaseItems = null, string userEmail = null);
